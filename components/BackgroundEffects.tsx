...
export default function BackgroundEffects() {
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0" aria-hidden="true">
      ...
      <div className={`absolute top-3/4 right-1/4 w-24 h-24 bg-red-400/5 dark:bg-red-400/8 rounded-full blur-xl opacity-20 dark:opacity-40 z-1 ${prefersReducedMotion ? "" : "animate-pulse"}`} style={{ animationDelay: "1s" }}></div>
      <div className={`absolute top-1/2 left-3/4 w-40 h-40 bg-red-600/3 dark:bg-red-600/6 rounded-full blur-xl opacity-15 dark:opacity-30 z-1 ${prefersReducedMotion ? "" : "animate-pulse"}`} style={{ animationDelay: "2s" }}></div>
      ...
      <div className="absolute bottom-0 left-0 w-full h-1/3 bg-gradient-to-t from-red-100/30 dark:from-red-950/20 to-transparent z-1"></div>
